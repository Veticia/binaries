...
def fetch_oculus_apps_with_covers(existing_apps):
            ...
            for edge in edges:
                ...
                try:
                    ...
                    if latest_supported_binary is not None:
                        ...
                        # save as png because Launcher doesn't care
                        executor.submit(download_image, landscape_url, os.path.join(landscape_folder, f"{package_name}.png"))
                        #executor.submit(download_image, portrait_url, os.path.join(portrait_folder, f"{package_name}.jpg"))
                        executor.submit(download_image, square_url, os.path.join(square_folder, f"{package_name}.png"))
    ...
    # Write the updated data to the oculus_apps.json file
    with open("oculus_apps.json", "w") as f:
        json.dump(merged_apps, f, indent=4)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future, app in zip(futures, app_data):
            try:
                ...
                square_filename = f"icons/{app['packageName']}.png"
                landscape_filename = f"banners/{app['packageName']}.png"
